<?php 
	...
	if(isset($_GET["id"]))
	{
		$data1=mysqli_query($connection,"select * from butiran where jenis='$_GET[id]'");
	}
	else
	{
				...
				<tr>
					...
					<form name="Form1">
						...
						<select name="example" size="1" onChange="go()">
							<?php 
								$data3 = mysqli_query($connection,"select * from jenis");
								if(isset($_GET["id"]))
								{
								...
								while ($info3=mysqli_fetch_array($data3)) 
								{
									...
									echo "<option value='laporan_jenis.php?id=$info3[id]&nama=$info3[jenis]'>$info3[jenis]</option>";
								}
							 ?>
						</select></p>
